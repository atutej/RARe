...
class LLM2VecWrapper:
    def __init__(self, use_ic_examples=False, *args, **kwargs):
        self.use_ic_examples = use_ic_examples
        try:
    ...
    def preprocess_for_ic(self, instruction, sentence):
        if not self.use_ic_examples:
            instruction = "Instruct: " + instruction
            sentence = "Query: " + sentence
            return [instruction, sentence]
        
        examples = sentence.split("Query:")
        instruction =  "Instruct: " + instruction + "\nQuery: " + "Query: ".join(examples[:-1])
        sentence = "Query: " + examples[-1]
        return [instruction, sentence]
    ...
    def encode(
        ...
        **kwargs: Any,  # noqa
        ...
        sentences = [self.preprocess_for_ic(instruction, sentence) for sentence in sentences]
...
llm2vec_llama3_8b_rare_hf = ModelMeta(
    loader=_loader(
        LLM2VecWrapper,
        use_ic_examples=True,
        max_length=4096,
        base_model_name_or_path="McGill-NLP/LLM2Vec-Meta-Llama-3-8B-Instruct-mntp",
        peft_model_name_or_path="McGill-NLP/LLM2Vec-Meta-Llama-3-8B-Instruct-mntp-supervised",
        second_peft_model_name_or_path="atutej/RARe-LLM2Vec-Llama-3-8B-Instruct-Ret",
        merge_peft=True,
        device_map="auto",
        torch_dtype=torch.bfloat16,
    ),
    name="RARe-LLM2Vec-Llama-3-8B-Instruct-Ret-hf",
    languages=["eng_Latn"],
    open_source=True,
    revision=None,
    release_date="2024-04-09",
