...
class LLM2Vec(nn.Module):
    ...
    def from_pretrained(
        ...
        second_peft_model_name_or_path=None,
        merge_peft=False,
        ...
        **kwargs,
        ...
        if second_peft_model_name_or_path is not None:
            model = PeftModel.from_pretrained(
                model,
                second_peft_model_name_or_path,
            )
            if merge_peft:
                model = model.merge_and_unload()
            
            config = {}
            config_addr = (second_peft_model_name_or_path)
        else:
            config = {}
            config_addr = (
                peft_model_name_or_path
